...
package net.mcreator.lulw.keybind;
...
import net.mcreator.lulw.procedures.GuiProcedure;
...
public class MKeyBinding extends LulwModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public MKeyBinding(LulwModElements instance) {
		super(instance, 12);
	...
	public void initElements() {
		keys = new KeyBinding("key.lulw.m", GLFW.GLFW_KEY_M, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					LulwMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
