...
package net.mcreator.mechanical_craft.keybind;
...
import net.mcreator.mechanical_craft.procedures.GuiProcedure;
...
public class MKeyBinding extends MechanicalCraftModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	...
	public void initElements() {
		keys = new KeyBinding("key.mechanical_craft.m", GLFW.GLFW_KEY_M, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					MechanicalCraftMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				GuiProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
